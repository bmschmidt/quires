...
export class GC {
	...
	stingyEval(c, untagged_c, info, type) {
		...
		switch (type) {
			case ClosureTypes.IND: {
				// Whitehole
				this.memory.i64Store(untagged_c, this.symbolTable.addressOf('stg_WHITEHOLE_info'));
				// Follow the indirectee
				const [res_c, _] = this.stingyEval(
					Number(this.memory.i64Load(untagged_c + rtsConstants.offset_StgInd_indirectee))
				);
			...
			case ClosureTypes.THUNK_SELECTOR: {
				...
				const [res_c, res_type] = this.stingyEval(
					Number(this.memory.i64Load(untagged_c + rtsConstants.offset_StgSelector_selectee))
				);
				...
				switch (res_type) {
					...
					case ClosureTypes.CONSTR_NOCAF: {
						const offset = this.memory.i32Load(info + rtsConstants.offset_StgInfoTable_layout);
						// Warning: at this point (and in the similar point below)
						// we may be losing the dynamic tagging, fixme
						const selectee = this.memory.i64Load(Memory.unDynTag(res_c) + ((1 + offset) << 3));
	...
	evacuateClosure(c) {
		...
		if (this.infoTables && !this.infoTables.has(info))
			throw new WebAssembly.RuntimeError(`Invalid info table 0x${info.toString(16)}`);
		...
		switch (type) {
			...
			case ClosureTypes.IND: {
				dest_c = this.evacuateClosure(
					this.memory.i64Load(untagged_c + rtsConstants.offset_StgInd_indirectee)
				);
				...
				dest_c = this.copyClosure(
					...
					Math.ceil(
						(rtsConstants.sizeof_StgArrBytes +
							Number(this.memory.i64Load(untagged_c + rtsConstants.offset_StgArrBytes_bytes))) /
							8
					) * 8
				);
				break;
			...
			case ClosureTypes.MUT_ARR_PTRS_FROZEN_CLEAN: {
				dest_c = this.copyClosure(
					...
					rtsConstants.sizeof_StgMutArrPtrs +
						(Number(this.memory.i64Load(untagged_c + rtsConstants.offset_StgMutArrPtrs_ptrs)) << 3)
				);
				break;
			}
			...
			case ClosureTypes.SMALL_MUT_ARR_PTRS_FROZEN_CLEAN: {
				dest_c = this.copyClosure(
					...
					rtsConstants.sizeof_StgSmallMutArrPtrs +
						(Number(
							this.memory.i64Load(untagged_c + rtsConstants.offset_StgSmallMutArrPtrs_ptrs)
						) <<
							3)
				);
				break;
	...
	scavengeSmallBitmap(payload, bitmap, size) {
		for (let i = 0; i < size; ++i)
			if (!(Number(bitmap >> BigInt(i)) & 1)) this.scavengeClosureAt(payload + (i << 3));
	}
	...
	scavengeLargeBitmap(payload, large_bitmap, size) {
		for (let j = 0; j < size; j += 64) {
			...
			for (let i = j; i - j < 64 && i < size; ++i)
				if (!(Number(bitmap >> BigInt(i - j)) & 1)) this.scavengeClosureAt(payload + (i << 3));
		}
	}
	...
	scavengePAP(c, offset_fun, payload, n_args) {
		...
		if (this.infoTables && !this.infoTables.has(fun_info))
			throw new WebAssembly.RuntimeError(`Invalid info table 0x${fun_info.toString(16)}`);
		switch (
			this.memory.i32Load(
				fun_info +
	...
	scavengeStackChunk(sp, sp_lim) {
		...
		while (true) {
			...
			if (this.memory.i32Load(info + rtsConstants.offset_StgInfoTable_srt))
				this.evacuateClosure(this.memory.i64Load(info + rtsConstants.offset_StgRetInfoTable_srt));
			switch (type) {
				...
				case ClosureTypes.RET_BIG: {
					const size = Number(
						this.memory.i64Load(Number(raw_layout) + rtsConstants.offset_StgLargeBitmap_size)
					);
				...
				case ClosureTypes.RET_FUN: {
					const retfun = c;
					const size = Number(this.memory.i64Load(retfun + rtsConstants.offset_StgRetFun_size));
		...
		while (true) {
			if (!currentBlock) {
				...
				if (currentBlock)
					...
					currentObject = Number(
						this.memory.i64Load(currentBlock + rtsConstants.offset_bdescr_start)
					);
			}
			...
			while (currentBlock) {
				...
				const currentLimit = Number(
					this.memory.i64Load(currentBlock + rtsConstants.offset_bdescr_free)
				);
	...
	scavengeClosure(c) {
		...
		switch (type) {
			...
			case ClosureTypes.THUNK_0_2: {
				if (this.memory.i32Load(info + rtsConstants.offset_StgInfoTable_srt))
					this.evacuateClosure(
						this.memory.i64Load(info + rtsConstants.offset_StgThunkInfoTable_srt)
					);
				const ptrs = this.memory.i32Load(info + rtsConstants.offset_StgInfoTable_layout),
					non_ptrs = this.memory.i32Load(info + rtsConstants.offset_StgInfoTable_layout + 4);
			...
			case ClosureTypes.AP_STACK: {
				...
				this.scavengeStackChunk(
					...
					c + rtsConstants.offset_StgAP_STACK_payload + size
				);
			...
			case ClosureTypes.ARR_WORDS: {
				return (
					Math.ceil(
						(rtsConstants.sizeof_StgArrBytes +
							Number(this.memory.i64Load(c + rtsConstants.offset_StgArrBytes_bytes))) /
							8
					) * 8
				);
			}
	...
	updateNursery() {
		...
		const base_reg =
			...
			hp_alloc = Number(this.memory.i64Load(base_reg + rtsConstants.offset_StgRegTable_rHpAlloc));
		// reset the number of allocated bytes in the nursery
		this.memory.i64Store(base_reg + rtsConstants.offset_StgRegTable_rHpAlloc, 0);
