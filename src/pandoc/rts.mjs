...
export async function newAsteriusInstance(req) {
	...
	let mkSptEntries = function (spt_offset_entries) {
		...
		for (const [k, off] of spt_offset_entries.entries()) {
			absolute_spt_entries.set(k, __asterius_memory_base.value + off);
		}
	...
	let mkInfoTable = function (offset_info_tables) {
		if (!(typeof offset_info_table === 'undefined')) {
			...
			for (const off of offset_info_tables.keys()) {
				absolute_info_tables.add(__asterius_memory_base.value + off);
			}
	...
	let __asterius_persistent_state = req.persistentState ? req.persistentState : {},
		...
		__asterius_static_mblocks = Math.ceil(
			(__asterius_memory_base.value + req.staticBytes) / rtsConstants.mblock_size
		),
		...
		__asterius_heapalloc = new HeapAlloc(__asterius_memory),
