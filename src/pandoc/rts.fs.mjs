class Device {
	...
	write(buf) {
		const str = typeof buf === 'string' ? buf : this.decoder.decode(buf, { stream: true });
...
export class FS {
	...
	read(fd, buf, count) {
		throw new WebAssembly.RuntimeError(`Attempting to read(${fd}, ${buf}, ${count})`);
	}
	...
	write(fd, buf, count) {
		...
		switch (fd) {
			...
			default: {
				throw new WebAssembly.RuntimeError(`Attempting to write(${fd}, ${buf}, ${count})`);
			}
		}
	}
	...
	history(fd) {
		switch (fd) {
			...
			default: {
				throw new WebAssembly.RuntimeError(`Attempting to get history of ${fd}`);
			}
		}
	}
}
