import type { Block, Inline } from '@djot/djot';
 ...
 */
export function matches(selector: string | undefined, element: Block | Inline): boolean {
...
function test(part: Selector, element: Block | Inline): boolean {
	...
	if (part.type === 'attribute') {
		...
		if (part.action === 'element') {
			return attributes[part.name]
				.toLocaleLowerCase()
				.split(' ')
				.includes(part.value.toLocaleLowerCase());
		}
