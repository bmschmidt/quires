...
!(function ($) {
	...
	var isIOSDevice = function () {
		...
		return (
			'string' == typeof e &&
			(-1 !== e.indexOf('iPhone') || -1 !== e.indexOf('iPad') || -1 !== e.indexOf('iPod'))
		);
	};
	$.extend($, {
		DEFAULT_SETTINGS: {
		...
		getElementOffset: function (e) {
			...
			return new $.Point(
				o.left + (i.pageXOffset || t.scrollLeft) - (t.clientLeft || 0),
		...
		setElementOpacity: function (e, t, i) {
			...
			if ($.Browser.opacity) e.style.opacity = t < 1 ? t : '';
			else if (t < 1) {
		...
		makeAjaxRequest: function (e, t, i) {
			...
			try {
				...
				if (window.XDomainRequest) {
					...
					if (c) {
						c.onload = function (e) {
							$.isFunction(t) && t({ responseText: c.responseText, status: 200, statusText: 'OK' });
						};
	...
	!(function () {
		...
		for (o = 0; o < r.length; o++)
			(n = (i = r[o]).indexOf('=')) > 0 &&
				(URLPARAMS[i.substring(0, n)] = decodeURIComponent(i.substring(n + 1)));
		$.Browser.alpha = !(
...
!(function (e) {
	...
	e.EventSource.prototype = {
		...
		addHandler: function (t, i, n) {
			var o = this.events[t];
			o || (this.events[t] = o = []);
			i && e.isFunction(i) && (o[o.length] = { handler: i, userData: n || null });
		},
...
!(function (e) {
	...
	e.MouseTracker = function (n) {
		...
		var o = arguments;
		e.isPlainObject(n) ||
			(n = { element: o[0], clickTimeThreshold: o[1], clickDistThreshold: o[2] });
		...
		this.dblClickTimeThreshold =
			n.dblClickTimeThreshold || e.DEFAULT_SETTINGS.dblClickTimeThreshold;
		this.dblClickDistThreshold =
			n.dblClickDistThreshold || e.DEFAULT_SETTINGS.dblClickDistThreshold;
			...
			mouseover: function (t) {
				!(function (t, i) {
					if (
						...
						m(i.currentTarget, i.relatedTarget)
					)
						return;
					v(t, i);
				})(r, t);
			...
			touchstart: function (i) {
				!(function (i, n) {
					...
					for (r = 0; r < t.length; r++)
						if (t[r] !== i && t[r].isTracking() && m(t[r].element, i.element)) {
							l = [];
							for (a = 0; a < c; a++)
								l.push({
	...
	e.MouseTracker.prototype = {
		...
		getActivePointersListsExceptType: function (e) {
			var t = i[this.hash];
			var n = [];
			for (var o = 0; o < t.activePointersLists.length; ++o)
				t.activePointersLists[o].type !== e && n.push(t.activePointersLists[o]);
			return n;
		},
	...
	if (
		window.PointerEvent &&
		(window.navigator.pointerEnabled || e.Browser.vendor !== e.BROWSERS.IE)
	) {
		...
		'ontouchstart' in window &&
			e.MouseTracker.subscribeEvents.push('touchstart', 'touchend', 'touchmove', 'touchcancel');
	...
	function n(t) {
		...
		for (n = 0; n < r; n++)
			if (o.activePointersLists[n].captureCount > 0) {
				...
				e.removeEvent(
					...
					e.MouseTracker.unprefixedPointerEvents ? 'pointermove' : 'MSPointerMove',
					o.pointermovecaptured,
					!0
				);
				e.removeEvent(
					e.MouseTracker.captureElement,
					e.MouseTracker.unprefixedPointerEvents ? 'pointerup' : 'MSPointerUp',
					o.pointerupcaptured,
					!0
				);
	...
	function k(e, t) {
		!(function (e, t, i) {
			L(e, t, i, 0);
			M(e, t, i);
		})(e, t, [{ id: t.pointerId, type: l(t) }]);
	}
	function B(e, t) {
		t.hasOwnProperty('isPrimary') ||
		...
		for (s = 0; s < p; s++) {
			...
			if (1 === c.contacts)
				t.pressHandler &&
					...
					e.cancelEvent(n);
			else if (2 === c.contacts && t.pinchHandler && 'touch' === a.type) {
				...
				h.lastPinchCenter = h.currentPinchCenter = d(
		...
		for (l = 0; l < v; l++) {
			...
			if ((c = m.getById(h.id))) {
				...
				if (w) {
					m.removeContact();
					(t.dragHandler || t.dragEndHandler || t.pinchHandler) &&
						e.MouseTracker.gesturePointVelocityTracker.removePoint(t, c);
					if (0 === m.contacts) {
						t.releaseHandler &&
							!1 ===
								t.releaseHandler({
						...
						if ((t.clickHandler || t.dblClickHandler) && c.insideElement) {
							p =
								a - c.contactTime <= t.clickTimeThreshold &&
								c.contactPos.distanceTo(s) <= t.clickDistThreshold;
							t.clickHandler &&
								!1 ===
									t.clickHandler({
										eventSource: t,
							...
							if (t.dblClickHandler && p) {
								...
								if (1 === m.clicks) {
									...
									m.clicks = 0;
									g.lastClickPos.distanceTo(s) <= t.dblClickDistThreshold &&
										!1 ===
											t.dblClickHandler({
												eventSource: t,
												pointerType: c.type,
						...
						g.lastPinchDist = g.currentPinchDist = g.pinchGPoints[0].currentPos.distanceTo(
							g.pinchGPoints[1].currentPos
						);
						g.lastPinchCenter = g.currentPinchCenter = d(
	...
	function F(t, n, o) {
		...
		if (0 === g.contacts)
			...
			if (
				t.pinchHandler &&
				'touch' === o[0].type &&
				(h = p.pinchGPoints[0].currentPos.distanceTo(p.pinchGPoints[1].currentPos)) !=
					p.currentPinchDist
			) {
...
!(function (e) {
	...
	e.Control = function (t, i, n) {
		...
		i.attachToViewer
			? this.anchor == e.ControlAnchor.TOP_RIGHT || this.anchor == e.ControlAnchor.BOTTOM_RIGHT
...
!(function (e) {
	...
	e.Viewer = function (o) {
		...
		this.world.addHandler('metrics-change', function (e) {
			h.viewport &&
				h.viewport._setContentBounds(h.world.getHomeBounds(), h.world.getContentFactor());
		});
		...
		for (r = 0; r < this.customControls.length; r++)
			this.addControl(this.customControls[r].id, { anchor: this.customControls[r].anchor });
	...
	e.extend(e.Viewer.prototype, e.EventSource.prototype, e.ControlDock.prototype, {
			...
			if (t)
				if (this.sequenceMode && e.isArray(t)) {
					...
					if (t.length) {
						...
						var h = function () {
							if (s + a === r)
								if (s) {
									...
									if (n.overlays && !n.preserveOverlays)
						...
						var c = function (t) {
							(e.isPlainObject(t) && t.tileSource) || (t = { tileSource: t });
							if (void 0 !== t.index) {
								e.console.error(
		...
		setFullPage: function (i) {
			...
			var h = { fullPage: i, preventDefaultAction: !1 };
			...
			if (i) {
				...
				this.toolbar &&
					...
					(this.element.style.height =
						e.getElementSize(this.element).y - e.getElementSize(this.toolbar.element).y + 'px');
		...
		addTiledImage: function (t) {
			...
			e.console.assert(
				!t.replace || (t.index > -1 && t.index < this.world.getItemCount()),
			...
			void 0 === t.ajaxHeaders || null === t.ajaxHeaders
				...
				: e.isPlainObject(t.ajaxHeaders) &&
				  ...
				  (t.ajaxHeaders = e.extend({}, this.ajaxHeaders, t.ajaxHeaders));
			var n = { options: t };
			function o(e) {
					...
					setTimeout(function () {
						if ('string' == e.type(i))
							(i = new e.TileSource({
								url: i,
								crossOriginPolicy:
									void 0 !== n.crossOriginPolicy ? n.crossOriginPolicy : t.crossOriginPolicy,
							...
							void 0 !== i.crossOriginPolicy ||
								(void 0 === n.crossOriginPolicy && void 0 === t.crossOriginPolicy) ||
								(i.crossOriginPolicy =
									void 0 !== n.crossOriginPolicy ? n.crossOriginPolicy : t.crossOriginPolicy);
							...
							if (e.isFunction(i.getTileUrl)) {
								...
								if (!h) {
									r({ message: 'Unable to load TileSource', source: i });
									return;
								}
		...
		addSimpleImage: function (t) {
			...
			var i = e.extend({}, t, { tileSource: { type: 'image', url: t.url } });
			delete i.url;
		...
		bindSequenceControls: function () {
			...
			if (this.showSequenceControl) {
				...
				if (s) {
					...
					this.toolbar
						? this.toolbar.addControl(this.pagingControl, { anchor: e.ControlAnchor.BOTTOM_RIGHT })
		...
		addOverlay: function (t, i, n, s) {
			...
			this.raiseEvent('add-overlay', { element: t, location: a.location, placement: a.placement });
			return this;
		},
		updateOverlay: function (i, n, o) {
			...
			if ((s = r(this.currentOverlays, i)) >= 0) {
				...
				t[this.hash].forceRedraw = !0;
				this.raiseEvent('update-overlay', { element: i, location: n, placement: o });
			}
			return this;
		},
		...
		e.requestAnimationFrame(function () {
			!(function (t) {
				...
				if (t.controlsShouldFade) {
					...
					o = Math.min(1, o);
					o = Math.max(0, o);
					for (r = t.controls.length - 1; r >= 0; r--)
	...
	function g(e) {
		...
		if (
			...
			this.viewport &&
			e.quick &&
			(t = this.gestureSettingsByDeviceType(e.pointerType)).clickToZoom
		) {
	...
	function R(t) {
		...
		if ((o = e.now()) - this._lastScrollTime > this.minScrollDeltaTime) {
			...
			if (
				...
				this.viewport &&
				(i = this.gestureSettingsByDeviceType(t.pointerType)).scrollToZoom
			) {
				n = Math.pow(this.zoomPerScroll, t.scroll);
				this.viewport.zoomBy(
					n,
					i.zoomToRefPoint ? this.viewport.pointFromPixel(t.position, !0) : null
				);
				this.viewport.applyConstraints();
			}
...
!(function (e) {
	e.Navigator = function (s) {
		...
		this.totalBorderWidths = new e.Point(2 * this.borderWidth, 2 * this.borderWidth).minus(
			this.fudge
		);
	...
	function t(e) {
		...
		if (
			...
			this.panVertical
				? this.panHorizontal || (i.x = this.viewer.viewport.getCenter(!0).x)
	...
	e.TileSource.prototype = {
		...
		getImageInfo: function (t) {
			var i,
				...
				h = this;
			t &&
				(l = (a = (s = t.split('/'))[s.length - 1]).lastIndexOf('.')) > -1 &&
				(s[s.length - 1] = a.slice(0, l));
			n = function (i) {
				'string' == typeof i && (i = e.parseXml(i));
				var n = e.TileSource.determineType(h, i, t);
				if (n) {
					void 0 === (r = n.prototype.configure.apply(h, [i, t])).ajaxWithCredentials &&
						(r.ajaxWithCredentials = h.ajaxWithCredentials);
					o = new n(r);
					h.ready = !0;
					h.raiseEvent('ready', { tileSource: o });
				} else h.raiseEvent('open-failed', { message: 'Unable to load TileSource', source: t });
			};
			if (t.match(/\.js$/)) {
				i = t.split('/').pop().replace('.js', '');
				e.jsonp({ url: t, async: !1, callbackName: i, callback: n });
			} else
				e.makeAjaxRequest({
					url: t,
					...
					success: function (t) {
						var i = (function (t) {
							...
							if (o.match(/\s*<.*/))
								try {
									n =
										t.responseXML && t.responseXML.documentElement ? t.responseXML : e.parseXml(o);
								} catch (e) {
									n = t.responseText;
								}
							else if (o.match(/\s*[\{\[].*/))
					...
					error: function (e, i) {
						...
						h.raiseEvent('open-failed', { message: n, source: t });
					}
				});
		},
	...
	e.DziTileSource = function (t, i, n, o, r, s, a, l, h) {
		...
		if (this.displayRects)
			for (c = this.displayRects.length - 1; c >= 0; c--)
				for (d = (u = this.displayRects[c]).minLevel; d <= u.maxLevel; d++) {
	...
	e.extend(e.DziTileSource.prototype, e.TileSource.prototype, {
		supports: function (e, t) {
			...
			e.Image
				? (i = e.Image.xmlns)
				: e.documentElement &&
				  (('Image' != e.documentElement.localName && 'Image' != e.documentElement.tagName) ||
						(i = e.documentElement.namespaceURI));
			return (
				-1 !== (i = (i || '').toLowerCase()).indexOf('schemas.microsoft.com/deepzoom/2008') ||
		...
		configure: function (i, n) {
			...
			o = e.isPlainObject(i)
				...
				: (function (i, n) {
						...
						if ('Image' == c)
							try {
								void 0 === (a = h.getElementsByTagName('Size')[0]) &&
									(a = h.getElementsByTagNameNS(u, 'Size')[0]);
								d = {
									Image: {
								...
								if (!e.imageFormatSupported(d.Image.Format))
									throw new Error(e.getString('Errors.ImageFormat', d.Image.Format.toUpperCase()));
								void 0 === (o = h.getElementsByTagName('DisplayRect')) &&
									(o = h.getElementsByTagNameNS(u, 'DisplayRect')[0]);
								for (l = 0; l < o.length; l++) {
									r = o[l];
									void 0 === (s = r.getElementsByTagName('Rect')[0]) &&
		...
		if (this.tile_width && this.tile_height) {
			...
			if (1 == this.tiles.length) {
				...
				for (var i = 0; i < this.tiles.length; i++)
					for (var n = 0; n < this.tiles[i].scaleFactors.length; n++) {
			...
			-1 ==
				...
				].indexOf(h[0]) || -1 != h.indexOf('sizeByW'))
		) {
	...
	e.extend(e.IIIFTileSource.prototype, e.TileSource.prototype, {
		supports: function (e, t) {
			return (
				...
				!(
					!e['@context'] ||
					('http://library.stanford.edu/iiif/image-api/1.1/context.json' != e['@context'] &&
						'http://iiif.io/api/image/1/context.json' != e['@context'])
				) ||
				!(
					!e.profile ||
					0 !== e.profile.indexOf('http://library.stanford.edu/iiif/image-api/compliance.html')
				) ||
				!!(e.identifier && e.width && e.height) ||
				!(
					!e.documentElement ||
					'info' != e.documentElement.tagName ||
					'http://library.stanford.edu/iiif/image-api/ns/' != e.documentElement.namespaceURI
				)
			);
		},
		...
		getTileWidth: function (t) {
			...
			return this.tileSizePerScaleFactor && this.tileSizePerScaleFactor[i]
		...
		getTileUrl: function (e, t, i) {
			...
			var o,
				...
				p,
				g,
				m = Math.pow(0.5, this.maxLevel - e),
		...
		i.levels = (function (t) {
			...
			for (n = 0; n < t.length; n++)
				(i = t[n]).height && i.width && i.url
					? o.push({ url: i.url, width: Number(i.width), height: Number(i.height) })
	...
	e.extend(e.LegacyTileSource.prototype, e.TileSource.prototype, {
		supports: function (e, t) {
			return (
				...
				(e.documentElement && 'legacy-image-pyramid' == e.documentElement.getAttribute('type'))
			);
		},
		configure: function (i, n) {
	...
	e.extend(e.ImageTileSource.prototype, e.TileSource.prototype, {
		...
		getImageInfo: function (t) {
			...
			e.addEvent(i, 'load', function () {
				n.width = Object.prototype.hasOwnProperty.call(i, 'naturalWidth')
					? i.naturalWidth
					: i.width;
				n.height = Object.prototype.hasOwnProperty.call(i, 'naturalHeight')
					? i.naturalHeight
					: i.height;
			...
			e.addEvent(i, 'error', function () {
				n.raiseEvent('open-failed', { message: 'Error loading image at ' + t, source: t });
			});
			i.src = t;
		},
		getLevelScale: function (e) {
		...
		_buildLevels: function () {
			var t = [
				{
					url: this._image.src,
					width: Object.prototype.hasOwnProperty.call(this._image, 'naturalWidth')
						? this._image.naturalWidth
						: this._image.width,
					height: Object.prototype.hasOwnProperty.call(this._image, 'naturalHeight')
			...
			var i = Object.prototype.hasOwnProperty.call(this._image, 'naturalWidth')
				? this._image.naturalWidth
				: this._image.width;
			var n = Object.prototype.hasOwnProperty.call(this._image, 'naturalHeight')
				? this._image.naturalHeight
				: this._image.height;
	...
	e.Button = function (t) {
		...
		this.tracker = new e.MouseTracker({
			...
			exitHandler: function (t) {
				...
				t.insideElementPressed && o.raiseEvent('exit', { originalEvent: t.originalEvent });
			},
			...
			releaseHandler: function (t) {
				if (t.insideElementPressed && t.insideElementReleased) {
					...
					o.raiseEvent('release', { originalEvent: t.originalEvent });
				} else t.insideElementPressed ? n(o, e.ButtonState.GROUP) : i(o, e.ButtonState.HOVER);
			},
			clickHandler: function (e) {
				e.quick && o.raiseEvent('click', { originalEvent: e.originalEvent });
			},
	...
	e.Rect.prototype = {
		...
		intersection: function (t) {
			...
			function y(t, n, o, r) {
				...
				return -i <= h && h <= 1 - i && -i <= c && c <= 1 - i
					? new e.Point(t.x + c * s.x, t.y + c * s.y)
					: null;
			}
...
!(function (e) {
	...
	e.ReferenceStrip = function (h) {
		...
		if (h.width && h.height) {
			...
			p.addControl(this.element, { anchor: e.ControlAnchor.BOTTOM_LEFT });
		} else if ('horizontal' == h.scroll) {
			this.element.style.width =
				g.x * h.sizeRatio * p.tileSources.length + 12 * p.tileSources.length + 'px';
			this.element.style.height = g.y * h.sizeRatio + 'px';
			p.addControl(this.element, { anchor: e.ControlAnchor.BOTTOM_LEFT });
		} else {
			this.element.style.height =
				g.y * h.sizeRatio * p.tileSources.length + 12 * p.tileSources.length + 'px';
			this.element.style.width = g.x * h.sizeRatio + 'px';
			p.addControl(this.element, { anchor: e.ControlAnchor.TOP_LEFT });
		}
	...
	function o(t, i, n) {
		...
		r = Math.ceil(i / o) + 5;
		for (
			h = r = (r = (s = Math.ceil((Math.abs(n) + i) / o) + 1) - r) < 0 ? 0 : r;
			h < s && h < t.panels.length;
			h++
		)
	...
	function a(e) {
		...
		switch (e.keyCode) {
			case 38:
				n.call(this, { eventSource: this.tracker, position: null, scroll: 1, shift: null });
				return !1;
			case 40:
			case 37:
				n.call(this, { eventSource: this.tracker, position: null, scroll: -1, shift: null });
...
!(function (e) {
	...
	e.Spring.prototype = {
		...
		setExponential: function (t) {
			...
			if (this._exponential) {
				e.console.assert(
					0 !== this.current.value && 0 !== this.target.value && 0 !== this.start.value,
		...
		update: function () {
			...
			var n =
				this.current.time >= this.target.time
					...
					: t +
					  (i - t) *
							((o = this.springStiffness),
							(r = (this.current.time - this.start.time) / (this.target.time - this.start.time)),
							(1 - Math.exp(o * -r)) / (1 - Math.exp(-o)));
			var o, r;
...
!(function (e) {
	function t(t) {
		e.extend(!0, this, { timeout: e.DEFAULT_SETTINGS.timeout, jobId: null }, t);
		this.image = null;
	}
	t.prototype = {
		errorMsg: null,
...
!(function (e) {
	e.Drawer = function (t) {
		...
		t.source &&
			e.console.error('[Drawer] options.source is no longer accepted; use TiledImage instead');
	...
	e.Drawer.prototype = {
		...
		blendSketch: function (t, i, n, o) {
			var r = t;
			e.isPlainObject(r) || (r = { opacity: t, scale: i, translate: n, compositeOperation: o });
...
!(function (e) {
	...
	e.Viewport.prototype = {
		...
		zoomTo: function (t, i, n) {
			...
			this.viewer && this.viewer.raiseEvent('zoom', { zoom: t, refPoint: i, immediately: n });
			return this;
		},
		...
		imageToWindowCoordinates: function (t) {
			...
			return this.imageToViewerElementCoordinates(t).plus(
		...
		viewportToWindowCoordinates: function (t) {
			...
			return this.viewportToViewerElementCoordinates(t).plus(
				e.getElementPosition(this.viewer.element)
			);
		},
...
!(function (e) {
	e.TiledImage = function (t) {
		...
		this._drawingHandler = function (t) {
			i.viewer.raiseEvent('tile-drawing', e.extend({ tiledImage: i }, t));
		};
	};
	e.extend(e.TiledImage.prototype, e.EventSource.prototype, {
		...
		_updateViewport: function () {
			...
			for (var p = h; p >= l; p--) {
				var g = !1;
				var m =
					i.deltaPixelsFromPointsNoRotate(this.source.getPixelRatio(p), !0).x *
					this._scaleSpring.current.value;
				...
				var v =
					i.deltaPixelsFromPointsNoRotate(this.source.getPixelRatio(p), !1).x *
					this._scaleSpring.current.value;
				var f =
				...
				c = t(this, u, g, p, Math.min(1, (m - 0.5) / 0.5), w / Math.abs(w - v), r, d, c);
			...
			!(function (t, i) {
				...
				n &&
					(o =
						t.opacity < 1 ||
						(t.compositeOperation && 'source-over' !== t.compositeOperation) ||
						(!t._isBottomItem() && n._hasTransparencyChannel()));
				var r;
				var s;
				...
				if (!r) {
					0 !== t.viewport.degrees
						? t._drawer._offsetForRotation({ degrees: t.viewport.degrees, useSketch: o })
				...
				for (var m = i.length - 1; m >= 0; m--) {
					...
					n.beingDrawn = !0;
					t.viewer && t.viewer.raiseEvent('tile-drawn', { tiledImage: t, tile: n });
				}
				c && t._drawer.restoreContext(o);
				if (!r) {
					t.getRotation(!0) % 360 != 0 && t._drawer._restoreRotationChanges(o);
					0 !== t.viewport.degrees
						? t._drawer._restoreRotationChanges(o)
						: t._drawer.viewer.viewport.flipped && t._drawer._flip({});
				}
				if (o) {
					if (r) {
						0 !== t.viewport.degrees &&
							t._drawer._offsetForRotation({ degrees: t.viewport.degrees, useSketch: !1 });
						t.getRotation(!0) % 360 != 0 &&
							t._drawer._offsetForRotation({
								degrees: t.getRotation(!0),
								point: t.viewport.pixelFromPointNoRotate(t._getRotationPoint(!0), !0),
		...
		_getCornerTiles: function (t, i, n) {
			...
			return { topLeft: h, bottomRight: c };
		}
	});
...
!(function (e) {
	...
	e.TileCache.prototype = {
		...
		cacheTile: function (i) {
			...
			})({ tile: i.tile, tiledImage: i.tiledImage });
		},
		clearTilesFor: function (t) {
			...
			for (var n = 0; n < this._tilesLoaded.length; ++n)
				if ((i = this._tilesLoaded[n]).tiledImage === t) {
					this._unloadTile(i);
	...
	e.extend(e.World.prototype, e.EventSource.prototype, {
		...
		setItemIndex: function (t, i) {
			...
			if (i !== n && -1 !== n) {
				...
				this._needsDraw = !0;
				this.raiseEvent('item-index-change', { item: t, previousIndex: n, newIndex: i });
			}
		},
		...
		arrange: function (t) {
			...
			for (var v = 0; v < this._items.length; v++) {
				...
				g =
					(p =
						(d = (u = this._items[v]).getBounds()).width > d.height
							? s
							: s * (d.width / d.height)) *
					(d.height / d.width);
		...
		_figureSizes: function () {
			...
			(this._contentFactor === n && this._homeBounds.equals(t) && this._contentSize.equals(i)) ||
